...
namespace DossylEditor.Utilities {
    ...
    static class Logger {
        ...
        ) { // have to sync with main thread (UI thread) because will get error if called on a non main thread
            await Application.Current.Dispatcher.BeginInvoke(new Action(() => {
                _messages.Add(new LogMessage(type, msg, file, caller, line));
            }));
        }
        public static async void Clear() { // have to sync with main thread
            await Application.Current.Dispatcher.BeginInvoke(new Action(() => {
                _messages.Clear();
            }));
        }
